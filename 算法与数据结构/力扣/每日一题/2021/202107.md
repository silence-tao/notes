...
## 20210722 <font color=#ed7336>M</font> <font color=#009975>S</font>

### 138. 复制带随机指针的链表

原题链接：[138. 复制带随机指针的链表](https://leetcode-cn.com/problems/copy-list-with-random-pointer/)
...
## 20210723 <font color=#009975>E</font> <font color=#009975>S</font>

### 1893. 检查是否区域内所有整数都被覆盖

原题链接：[1893. 检查是否区域内所有整数都被覆盖](https://leetcode-cn.com/problems/check-if-all-the-integers-in-a-range-are-covered/)
    ...
    public boolean isCovered(int[][] ranges, int left, int right) {
        ...
        for (int[] item : ranges) {
            ...
            if (item[0] <= left && item[1] >= left) {
                // 如果在区间内，left就往右边缩小至item[1] + 1
                left = item[1] + 1;
            }
        }

        // 如果left缩小到了right的右边
        // 说明区间[left, right]被 ranges 中至少一个区间覆盖
        return left > right;
    }
}
```

## 20210724 <font color=#009975>E</font> <font color=#009975>S</font>

### 1736. 替换隐藏数字得到的最晚时间

原题链接：[1736. 替换隐藏数字得到的最晚时间](https://leetcode-cn.com/problems/latest-time-by-replacing-hidden-digits/)
...
class Solution {
    public String maximumTime(String time) {
        ...
        if (b == '?') {
            // 如果小时第一位是0或1，那么第二位最大就是9
            if (a == '0' || a == '1') {
                b = '9';
            } else {
            	// 如果小时第一位是2，那么第二位最大就是2
                b = '3';
            }
        } else if (b > '3' && a == '2') {
            // 如果挡第二位是确定的并且大于3并且第一位为2
            // 那么第一位最大为1
            a = '1';
        }
...
## 20210725 <font color=#ed7336>M</font> <font color=#ed7336>R</font>

### 1743. 从相邻元素对还原数组

原题链接：[1743. 从相邻元素对还原数组](https://leetcode-cn.com/problems/restore-the-array-from-adjacent-pairs/)
...
[参考题解](https://leetcode-cn.com/problems/restore-the-array-from-adjacent-pairs/solution/gong-shui-san-xie-yi-ti-shuang-jie-dan-x-elpx/)

``` java
class Solution {
    public int[] restoreArray(int[][] adjacentPairs) {
        int length = adjacentPairs.length, n = length + 1;

        // 每个元素在adjacentPairs中出现次数的的映射关系
        // 整数数组中只有nums[0]和nums[n - 1]在adjacentPairs只会出现一层
        Map<Integer, Integer> countMap = new HashMap<>();
        // 元素和它相邻元素的映射关系
        Map<Integer, List<Integer>> nearMap = new HashMap<>();
        for (int[] item : adjacentPairs) {
            int num1 = item[0], num2 = item[1];

            // 统计元素在adjacentPairs出现的次数
            countMap.put(num1, countMap.getOrDefault(num1, 0) + 1);
            countMap.put(num2, countMap.getOrDefault(num2, 0) + 1);

            // 统计每个元素相邻的元素
            List<Integer> list = nearMap.get(num1);
            if (list == null) {
                list = new ArrayList<>();
                nearMap.put(num1, list);
            }
            list.add(num2);
         
            list = nearMap.get(num2);
            if (list == null) {
                list = new ArrayList<>();
                nearMap.put(num2, list);
            }
            list.add(num1);   
        }
        ...
        int[] res = new int[n];
        res[0] = start;
        res[1] = nearMap.get(start).get(0);
        // 从nearMap中按顺序找到其它元素
        for (int i = 2; i < n; i++) {
            for (int j : nearMap.get(res[i - 1])) {
                if (j != res[i - 2]) {
                    res[i] = j;
                }
            }
...
## 20210726 <font color=#ed7336>M</font> <font color=#ed7336>R</font>

### 1713. 得到子序列的最少操作次数

原题链接：[1713. 得到子序列的最少操作次数](https://leetcode-cn.com/problems/minimum-operations-to-make-a-subsequence/)
...
[参考题解](https://leetcode-cn.com/problems/minimum-operations-to-make-a-subsequence/solution/de-dao-zi-xu-lie-de-zui-shao-cao-zuo-ci-hefgl/)
...
class Solution {
    public int minOperations(int[] target, int[] arr) {
        int n = target.length;
        // 因为数组target没有相同的元素
        // 先将数组target中的元素与下标的关系保存在map中
        Map<Integer, Integer> map = new HashMap<>();
        for (int i = 0; i < n; i++) {
            map.put(target[i], i);
        }

        // 然后按元素在arr的出现顺序对应在target中的顺序放入到list中
        List<Integer> list = new ArrayList<>();
        for (int item : arr) {
            if (map.containsKey(item)) {
                // 找到元素item在target数组中的顺序
                int x = map.get(item);
                // 利用二分搜索查找x在list中的位置
                int pos = binarySearch(list, x);
                // pos != list.size()表示在中间
                if (pos != list.size()) {
                    list.set(pos, x);
                } else {
...
## 20210727 <font color=#009975>E</font> <font color=#009975>S</font>

### 671. 二叉树中第二小的节点

原题链接：[671. 二叉树中第二小的节点](https://leetcode-cn.com/problems/second-minimum-node-in-a-binary-tree/)
...
class Solution {
    public int findSecondMinimumValue(TreeNode root) {
        // min表示最小的值，second表示第二小的值
        int min = root.val, second = root.val;
        // 定义LinkedList作为队列
        ...
        while (!queue.isEmpty()) {
            ...
            if (node.val < min) {
                ...
                // 最小值min更新为node.val
                min = node.val;
            } else if ((min < node.val && node.val < second) || min == second) {
                // 如果当前节点值介于min和second之间，说明second已经不是第二小的值
                // 或者min和second相等
                // 那么将second更新为node.val
                second = node.val;
            }
        ...
        // min == second表示所以节点值都相等，也就是没有第二小的值
        // 否则第二小
...
## 20210728 <font color=#ed7336>M</font> <font color=#ec4c47>U</font>

### 863. 二叉树中所有距离为 K 的结点

原题链接：[863. 二叉树中所有距离为 K 的结点](https://leetcode-cn.com/problems/all-nodes-distance-k-in-binary-tree/)
...
[参考题解](https://leetcode-cn.com/problems/all-nodes-distance-k-in-binary-tree/solution/er-cha-shu-zhong-suo-you-ju-chi-wei-k-de-qbla/)
...
## 20210729 <font color=#ed7336>M</font> <font color=#009975>S</font>

### 1104. 二叉树寻路

原题链接：[1104. 二叉树寻路](https://leetcode-cn.com/problems/path-in-zigzag-labelled-binary-tree/)
...
>
> ![image-2021072923100062.png](../../../../img/image-2021072923100062.png)
>
...
class Solution {
    public List<Integer> pathInZigZagTree(int label) {
        // 先求出label所在的层数
        int row = (int) (Math.log(label) / Math.log(2)) + 1, i = row;
        
        // 反向求出到label到root节点的路径
        int[] arr = new int[row + 1];
        while (i > 0) {
            arr[i] = label;

            // label“之”字形的父节点就是当前label节点所在层的对称节点实际的父节点
            // (1 << (i - 1)) 表示当前层最小的节点值
            // (1 << i) - 1 表示当前层最大的节点值
            // (1 << (i - 1)) + ((1 << i) - 1 - label) 表示的是label的对称节点
            // 除以算出了label对称节点实际的父节点
            // 也就是label“之”字形的父节点
            label = ((1 << (i - 1)) + ((1 << i) - 1 - label)) / 2;
...
## 20210730 <font color=#009975>E</font> <font color=#009975>S</font>

### 171. Excel表列序号

原题链接：[171. Excel表列序号](https://leetcode-cn.com/problems/excel-sheet-column-number/)
...
## 20210731 <font color=#ec4c47>H</font> <font color=#009975>S</font>

### 987. 二叉树的垂序遍历

原题链接：[987. 二叉树的垂序遍历](https://leetcode-cn.com/problems/vertical-order-traversal-of-a-binary-tree/)
...
class Solution {
     ...
     * 用于保存节点坐标的数据结构
     */
    public static class TreeNodePlus {
        // x，y坐标
        int x;
        int y;
        // node节点
        ...
        TreeNodePlus(int x, int y, TreeNode node) {
            this.x = x;
            this.y = y;
            this.node = node;
        }
    ...
    public List<List<Integer>> verticalTraversal(TreeNode root) {
        // 用来做二叉树的层序遍历
        LinkedList<TreeNodePlus> queue = new LinkedList<>();
        // 将根节点加入队列
        queue.addFirst(new TreeNodePlus(0, 0, root));

        // 用哈希保存每一列的节点值的集合
        Map<Integer, List<Integer>> map = new HashMap<>();
        // min和max用来记录列值的最小值和最大值
        int min = 1000, max = -1000;
        // 二叉树的层序遍历
        while (!queue.isEmpty()) {
            // 为了保证每一层中同一列的值有序，所以需要将同一层的节点同时遍历
            // 这里用集合收集同一层的节点，统一遍历
            List<TreeNodePlus> nodePlusList = new ArrayList<>();
            while (!queue.isEmpty()) {
                nodePlusList.add(queue.removeLast());
            }

            // 用哈希tempMap保存同一层中每一列节点值的集合
            Map<Integer, List<Integer>> tempMap = new HashMap<>();
            // 遍历当前层的节点
            for (TreeNodePlus nodePlus : nodePlusList) {
                // 更新列值的最小值
                if (nodePlus.y < min) {
                    min = nodePlus.y;
                }
                ...
                TreeNode node = nodePlus.node;
                // 获取当前列的节点值集合
                List<Integer> list = tempMap.get(nodePlus.y);
                // 如果节点值集合为空
                if (list == null) {
                    // 就创建集合
                    list = new ArrayList<>();
                    // 并将映射关系保存到tempMap中
                    tempMap.put(nodePlus.y, list);
                }
                ...
                if (node.left != null) {
                    // 将节点的左孩子加入集合
                    queue.addFirst(new TreeNodePlus(nodePlus.x + 1, nodePlus.y - 1, node.left));
                }

                // 如果节点的右孩子节点不为空
                if (node.right != null) {
                    // 将节点的右孩子加入集合
                    queue.addFirst(new TreeNodePlus(nodePlus.x + 1, nodePlus.y + 1, node.right));
                }
            }
